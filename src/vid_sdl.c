...
void IN_Commands (void)
{
    ...
    QuakeWeb_PollGamepad(&mouse_x, &mouse_y);
...
{
    /* Mouselook is forced on. Vanilla gates it behind a held +mlook key and
       otherwise walks the player with mouse Y, which is unusable under pointer
       lock where there is no way to stop moving the mouse. */
    if (!mouse_avail)
        return;
